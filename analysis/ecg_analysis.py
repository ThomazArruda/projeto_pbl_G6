"""
Análise Estatística de Dados de ECG - Sessões 19-23
Script para análise da variação ECG (delta max-min) entre pernas esquerda e direita
...
class ECGDeltaAnalyzer:
    """Classe para análise de variação ECG e teste de normalidade"""
    ...
    def calculate_deltas(self):
        """
        Calcula a variação (delta) de ECG para cada sessão
        Delta = ECG_máximo - ECG_mínimo
        """
        print(f"\n{'='*80}")
        print(f"CÁLCULO DE VARIAÇÃO ECG (DELTA)")
        print(f"{'='*80}\n")
        ...
        for session_id, data in sorted(self.sessions_data.items()):
            try:
                ...
                if isinstance(raw_data, list):
                    # Extrair ECG de cada ponto de dados
                    for point in raw_data:
                        if isinstance(point, dict):
                            if 'ESQ_ecg' in point:
                                ecg_esq_list.append(point['ESQ_ecg'])
                            if 'DIR_ecg' in point:
                                ecg_dir_list.append(point['DIR_ecg'])
                else:
                    # Fallback para dicionário (compatibilidade)
                    ecg_esq_list = raw_data.get('ESQ_ecg', [])
                    ecg_dir_list = raw_data.get('DIR_ecg', [])
                
                ecg_esq = np.array(ecg_esq_list)
                ecg_dir = np.array(ecg_dir_list)
                
                if len(ecg_esq) == 0 or len(ecg_dir) == 0:
                    print(f"  Sessão {session_id}: ✗ Dados de ECG incompletos")
                    continue
                
                # Calcular deltas
                delta_esq = np.max(ecg_esq) - np.min(ecg_esq)
                delta_dir = np.max(ecg_dir) - np.min(ecg_dir)
                
                self.deltas_esq.append(delta_esq)
                self.deltas_dir.append(delta_dir)
                
                print(f"  Sessão {session_id}:")
                print(f"    - Perna Esquerda: ΔECG = {delta_esq:.2f} mV (Min: {np.min(ecg_esq):.2f} mV, Max: {np.max(ecg_esq):.2f} mV)")
                print(f"    - Perna Direita:  ΔECG = {delta_dir:.2f} mV (Min: {np.min(ecg_dir):.2f} mV, Max: {np.max(ecg_dir):.2f} mV)")
    ...
    def generate_plots(self, output_dir="./"):
        ...
        ax.set_ylabel('Variação ECG (mV)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Pernas', fontsize=12, fontweight='bold')
        ax.set_title('Boxplot: Distribuição de Deltas ECG', fontsize=14, fontweight='bold')
        ...
        plot_file = output_path / f"boxplot_deltas_ecg_{timestamp}.png"
    ...
    def export_to_csv(self, output_dir="./"):
        ...
        df_sessions = pd.DataFrame({
            'Sessão': session_ids,
            'Delta_Esquerda (mV)': [round(d, 2) for d in self.deltas_esq],
            'Delta_Direita (mV)': [round(d, 2) for d in self.deltas_dir],
            'Diferença_DIR-ESQ (mV)': [round(self.deltas_dir[i] - self.deltas_esq[i], 2) for i in range(len(session_ids))]
        })
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        excel_file = output_path / f"analise_ecg_completa_{timestamp}.xlsx"
        ...
        csv_file = output_path / f"analise_ecg_resultados_{timestamp}.csv"
        
        # Criar um CSV consolidado bem formatado
        with open(csv_file, 'w', encoding='utf-8') as f:
            f.write("ANÁLISE ESTATÍSTICA DE VARIAÇÃO ECG - SESSÕES 19-23\n")
            f.write("="*80 + "\n\n")
            ...
            f.write("- Delta (ΔECG): Variação = ECG Máximo - ECG Mínimo\n")
    ...
    def print_summary(self):
        ...
        print("RESUMO DOS DELTAS ECG:")
        print(f"  Perna Esquerda (Parética):")
        print(f"    - Média de variação: {np.mean(self.deltas_esq):.2f} mV")
        print(f"    - Desvio padrão: {np.std(self.deltas_esq, ddof=1):.2f} mV")
        print(f"    - Intervalo: [{np.min(self.deltas_esq):.2f} mV, {np.max(self.deltas_esq):.2f} mV]")
        
        print(f"\n  Perna Direita (Controle):")
        print(f"    - Média de variação: {np.mean(self.deltas_dir):.2f} mV")
        print(f"    - Desvio padrão: {np.std(self.deltas_dir, ddof=1):.2f} mV")
        print(f"    - Intervalo: [{np.min(self.deltas_dir):.2f} mV, {np.max(self.deltas_dir):.2f} mV]")
        
        print(f"\n  Diferença Média (DIR - ESQ): {np.mean(self.deltas_dir) - np.mean(self.deltas_esq):.2f} mV\n")
...
def main():
    ...
    print("\n" + "="*80)
    print("ANÁLISE ESTATÍSTICA DE VARIAÇÃO ECG - PROJETO PBL")
    print("="*80 + "\n")
    
    # Inicializar analisador
    analyzer = ECGDeltaAnalyzer(db_path="../backend/clinic.db")
