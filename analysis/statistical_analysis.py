"""
Análise Estatística de Dados de Ângulos - Sessões 19-23
Script para análise da variação angular (delta max-min) entre pernas esquerda e direita
...
class AngleDeltaAnalyzer:
    """Classe para análise de variação angular e teste de normalidade"""
    ...
    def calculate_deltas(self):
        """
        Calcula a variação (delta) de ângulos para cada sessão
        Delta = ângulo_máximo - ângulo_mínimo
        """
        print(f"\n{'='*80}")
        print(f"CÁLCULO DE VARIAÇÃO ANGULAR (DELTA)")
        print(f"{'='*80}\n")
        ...
        for session_id, data in sorted(self.sessions_data.items()):
            try:
                ...
                if isinstance(raw_data, list):
                    # Extrair ângulos de cada ponto de dados
                    for point in raw_data:
                        if isinstance(point, dict):
                            if 'ESQ_angle' in point:
                                angles_esq_list.append(point['ESQ_angle'])
                            if 'DIR_angle' in point:
                                angles_dir_list.append(point['DIR_angle'])
                else:
                    # Fallback para dicionário (compatibilidade)
                    angles_esq_list = raw_data.get('ESQ_angle', [])
                    angles_dir_list = raw_data.get('DIR_angle', [])
                
                angles_esq = np.array(angles_esq_list)
                angles_dir = np.array(angles_dir_list)
                
                if len(angles_esq) == 0 or len(angles_dir) == 0:
                    print(f"  Sessão {session_id}: ✗ Dados de ângulo incompletos")
                    continue
                
                # Calcular deltas
                delta_esq = np.max(angles_esq) - np.min(angles_esq)
                delta_dir = np.max(angles_dir) - np.min(angles_dir)
                
                self.deltas_esq.append(delta_esq)
                self.deltas_dir.append(delta_dir)
                
                print(f"  Sessão {session_id}:")
                print(f"    - Perna Esquerda: ΔAngle = {delta_esq:.2f}° (Min: {np.min(angles_esq):.2f}°, Max: {np.max(angles_esq):.2f}°)")
                print(f"    - Perna Direita:  ΔAngle = {delta_dir:.2f}° (Min: {np.min(angles_dir):.2f}°, Max: {np.max(angles_dir):.2f}°)")
    ...
    def generate_plots(self, output_dir="./"):
        ...
        ax.set_ylabel('Variação Angular (°)', fontsize=12, fontweight='bold')
        ax.set_xlabel('Pernas', fontsize=12, fontweight='bold')
        ax.set_title('Boxplot: Distribuição de Deltas Angulares', fontsize=14, fontweight='bold')
        ...
        plot_file = output_path / f"boxplot_deltas_angulares_{timestamp}.png"
    ...
    def export_to_csv(self, output_dir="./"):
        ...
        df_sessions = pd.DataFrame({
            'Sessão': session_ids,
            'Delta_Esquerda (°)': [round(d, 2) for d in self.deltas_esq],
            'Delta_Direita (°)': [round(d, 2) for d in self.deltas_dir],
            'Diferença_DIR-ESQ (°)': [round(self.deltas_dir[i] - self.deltas_esq[i], 2) for i in range(len(session_ids))]
        })
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        excel_file = output_path / f"analise_angular_completa_{timestamp}.xlsx"
        ...
        csv_file = output_path / f"analise_angular_resultados_{timestamp}.csv"
        
        # Criar um CSV consolidado bem formatado
        with open(csv_file, 'w', encoding='utf-8') as f:
            f.write("ANÁLISE ESTATÍSTICA DE VARIAÇÃO ANGULAR - SESSÕES 19-23\n")
            f.write("="*80 + "\n\n")
            ...
            f.write("- Delta (ΔAngle): Variação = Ângulo Máximo - Ângulo Mínimo\n")
    ...
    def print_summary(self):
        ...
        print("RESUMO DOS DELTAS ANGULARES:")
        print(f"  Perna Esquerda (Parética):")
        print(f"    - Média de variação: {np.mean(self.deltas_esq):.2f}°")
        print(f"    - Desvio padrão: {np.std(self.deltas_esq, ddof=1):.2f}°")
        print(f"    - Intervalo: [{np.min(self.deltas_esq):.2f}°, {np.max(self.deltas_esq):.2f}°]")
        
        print(f"\n  Perna Direita (Controle):")
        print(f"    - Média de variação: {np.mean(self.deltas_dir):.2f}°")
        print(f"    - Desvio padrão: {np.std(self.deltas_dir, ddof=1):.2f}°")
        print(f"    - Intervalo: [{np.min(self.deltas_dir):.2f}°, {np.max(self.deltas_dir):.2f}°]")
        
        print(f"\n  Diferença Média (DIR - ESQ): {np.mean(self.deltas_dir) - np.mean(self.deltas_esq):.2f}°\n")
...
def main():
    ...
    print("\n" + "="*80)
    print("ANÁLISE ESTATÍSTICA DE VARIAÇÃO ANGULAR - PROJETO PBL")
    print("="*80 + "\n")
    
    # Inicializar analisador
    analyzer = AngleDeltaAnalyzer(db_path="../backend/clinic.db")
